...
class CompanyDataConverter
{
    ...
    protected function convertInstallments(): void
    {
        ...
        $this->insertWithIdentity('deduction_batches', $this->sourceQuery('hafithas')->map(fn ($row) => [
            ...
            'total_amount' => $row->tot ?? 0,
            'posted_normal_amount' => $row->morahel ?? 0,
            'posted_archive_amount' => $row->over_kst_arc ?? 0,
            'posted_surplus_amount' => $row->over_kst ?? 0,
            'posted_partial_amount' => $row->half ?? 0,
            'wrong_amount' => $row->wrong_kst ?? 0,
            'notes' => null,
            'created_by' => $row->user_id,
            'created_at' => $row->created_at,
            'updated_at' => $row->updated_at,
        ...
        $this->insertWithIdentity('deduction_batch_lines', $this->sourceQuery('hafitha_trans')->map(fn ($row) => [
            ...
            'deduction_date' => $row->ksm_date,
            'notes' => $row->ksm_notes,
            'entry_type' => (int) ($row->haf_kst_type ?? 1),
            'posted_type' => (int) ($row->haf_kst_type ?? 1),
            'created_by' => $row->user_id,
            'created_at' => $row->created_at,
            'updated_at' => $row->updated_at,
        ])->all());
