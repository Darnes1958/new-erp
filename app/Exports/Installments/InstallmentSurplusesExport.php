...
class InstallmentSurplusesExport implements FromCollection, WithHeadings, WithMapping, WithEvents
{
    ...
    /**
     * @param  InstallmentSurplus  $row
     * @return array<int, string|int|null>
     */
    public function map($row): array
    {
        return [
            Utf8Text::clean($row->contractable?->customer?->name),
            (string) $row->contractable_id,
            $row->surplus_date?->format('Y-m-d'),
    ...
    public function registerEvents(): array
    {
        ...
        return $this->rtlSheetEvents(
            ...
            totalAmount: (float) $this->rows->sum(fn (InstallmentSurplus $row): float => (float) $row->amount),
