...
class SupplierBalancesReportPage extends Page implements HasActions, HasForms, HasTable
{
    ...
    protected static ?string $title = 'أرصدة الموردين';

    protected static ?string $slug = 'supplier-balances-report';
    ...
    {
        return (string) $record->supplier_id;
    }
    ...
    {
        return app(SupplierLedgerReportService::class)->balancesQuery(
            $this->dateFrom,
            $this->dateTo,
            $this->includeZero,
        );
    ...
    protected function downloadExcel(): mixed
    {
        ...
        $title = 'أرصدة الموردين من تاريخ '.$this->dateFrom.' إلي تاريخ '.$this->dateTo;

        return app(MarketExcelService::class)->supplierBalances(
            rows: $rows,
            reportTitle: $title,
            summary: $summary,
        );
