...
class WrongDeductionReportTitle
{
    public static function build(?string $dateFrom, ?string $dateTo): string
    {
        ...
        if ($hasFrom && $hasTo) {
            ...
            return "تقرير بالأقساط الواردة بالخطأ من تاريخ : {$dateFrom} وحتي تاريخ {$untilFormatted}";
        }

        if ($hasFrom) {
            return "تقرير بالأقساط الواردة بالخطأ من تاريخ : {$dateFrom}";
        }

        if ($hasTo) {
            return "تقرير بالأقساط الواردة بالخطأ حتي تاريخ : {$dateTo}";
        }

        return 'تقرير بالأقساط الواردة بالخطأ حتى تاريخ: '.now()->toDateString();
    }
    ...
    public static function dateFilterState(object $livewire): array
    {
        ...
        $state = $livewire->getTableFilterState('deduction_date') ?? [];
