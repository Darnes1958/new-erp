...
class InstallmentSurplusReportTitle
{
    public static function build(?string $dateFrom, ?string $dateTo): string
    {
        ...
        if ($hasFrom && $hasTo) {
            ...
            return "تقرير بالأقساط المخصومة بالفائض من تاريخ : {$dateFrom} وحتي تاريخ {$untilFormatted}";
        }

        if ($hasFrom) {
            return "تقرير بالأقساط المخصومة بالفائض من تاريخ : {$dateFrom}";
        }

        if ($hasTo) {
            return "تقرير بالأقساط المخصومة بالفائض حتي تاريخ : {$dateTo}";
        }

        return 'تقرير بالأقساط المخصومة بالفائض حتى تاريخ: '.now()->toDateString();
    }
    ...
    public static function dateFilterState(object $livewire): array
    {
        ...
        $state = $livewire->getTableFilterState('surplus_date') ?? [];
